...
class _AppDashboardScreenState extends State<AppDashboardScreen>
  ...
  Future<void> _initDashboard() async {
    final bool isValid = await AppServices.validatePunchRequirements();
    if (!isValid) {
      if (mounted) {
        CommonSnackBar.show(
          context: context,
  ...
  Future<void> _fetchModules() async {
    try {
      ...
      setState(() => isLoadingModules = false);
      debugPrint("_fetchModules error: $e");
    }
  }
  ...
  Future<void> _fetchLastLoginDetails() async {
    try {
      ...
      debugPrint("_fetchLastLoginDetails error: $e");
    }
  }

  Future<void> _fetchWallPostData() async {
    try {
      final response = await ApiClient.get(
        ApiConstants.getWallPosts,
        query: {"Emp_PK": emppk},
      ...
      debugPrint("_fetchWallPostData error: $e");
    }
  }
