...
import 'package:new_design_demo/core/api/api_constants.dart';
import 'package:new_design_demo/core/app_services/PunchInOut/punch_service.dart';
import 'package:new_design_demo/core/app_services/device_other_details.dart';
...
class PunchController {
  ...
  PunchController({required this.context, required this.onStateChanged});
  ...
  Future<void> loadUserPrefs() async {
    ...
    // _deviceId = prefs.getString('deviceId') ?? 'DEVICE';  //GETTING DEVICE ID AT PUNCH TIME
    debugPrint(
      "PUNCH PREFS => $_empPk $_employeeCode $_locationPk $_companyPk",
    );
  }
  ...
  Future<void> getTodaysAttendanceState({required int emppk}) async {
    try {
      ...
      final response = await ApiClient.post(
        ...
        data: {"Emp_PK": emppk.toString(), "AttendanceDate": date},
      );

      if (response.statusCode == 200) {
        _parseAttendance(response.data);
      }
    } catch (e) {
  ...
  ////ATTENDANCE API RESULT HANDLES HERE
  void _parseAttendance(dynamic data) {
    try {
      inTimeVal = '';
      outTimeVal = '';

      final result = data["TodaysAttendanceResult"];
      ...
      inTimeVal = (row["InTime"] ?? '').toString().trim();

      outTimeVal = (row["OutTime"] ?? '').toString().trim();
  ...
  Future<void> onPunchTapped({required bool isPunchIn}) async {
    ...
    // IN PUNCH VALIDATION
    if (isPunchIn && inTimeVal.isNotEmpty) {
      CommonSnackBar.show(
        context: context,
    ...
    // OUT PUNCH VALIDATION
    if (!isPunchIn && inTimeVal.isEmpty) {
      CommonSnackBar.show(
        context: context,
        title: "Punch In Required",
        message: "Please punch in before punch out.",
        type: SnackBarType.error,
      );
  ...
  Future<void> _performPunch({required bool isPunchIn}) async {
    try {
      final position = await _getCurrLocPermission();

      await _getPunchExtraDetails(position);

      final now = _currentTimeFormatted();

      final today = DateFormat("dd/MM/yyyy").format(DateTime.now());

      final request = PunchRequestData(
        empPk: _empPk,
        ...
        batteryLevel: _batteryPercentage,
        ...
        // outTime: outTimeVal.isNotEmpty ? outTimeVal : "",
        inOrOut: isPunchIn ? '0' : '1',
      );

      final msg = isPunchIn
          ? await PunchService.punchIn(request)
          : await PunchService.punchOut(request);

      if (_isSuccessResponse(msg)) {
        if (isPunchIn) {
          inTimeVal = now;
          outTimeVal = '';
        } else {
        ...
        CommonSnackBar.show(
          ...
          message: isPunchIn ? "Punch In : $now" : "Punch Out : $now",
          type: SnackBarType.success,
        );
      } else {
        CommonSnackBar.show(
  ...
  // LOCATION
  Future<Position> _getCurrLocPermission() async {
  ...
  Future<void> _getPunchExtraDetails(Position position) async {
    _punchAddr = "${position.latitude},${position.longitude}";

    //  battery and device ID from DeviceInfoServiceClass
    final deviceInfo = await DeviceInfoService().getDeviceDetails();
    _batteryPercentage = deviceInfo['battery']?.toString() ?? '';
    _deviceId = deviceInfo['deviceId']?.toString() ?? '';
  }
